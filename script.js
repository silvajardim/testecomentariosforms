document.addEventListener('DOMContentLoaded', function () {
  ...
  if (btn && menu) {
    ...
    fetch(`https://docs.google.com/spreadsheets/d/1kQDuvDfQHIDjSV-4rDfz0d4uWdfzjhLvkpPcg6zN1lE/export?format=csv`)
  .then(response => response.text())
  .then(csvText => {
    const resultado = Papa.parse(csvText, { header: true });
    const tabela = document.querySelector('#tabela-comentarios tbody');

    resultado.data.forEach(linha => {
      if (linha.Nome && linha.Comentario && linha.Data) {
